...
(function() {
	...
	class ListServices {
		...
		static defaults = {
			...
			margin:			30,		// расстояние между элементами [px]
			...
			keyScrollButtons: 	false,	// управление клавишами вправо / влево
		...
		constructor(listServices, setup) {

			this.listServices = listServices;
			this.setup = setup;

			// контейнер в котором отображаются элементы галереи
			this.slider = this.listServices.querySelector('.slider-services');

			// контейнер, непосредственно в котором расположены элементы слайдера
			this.stage = this.listServices.querySelector('.stage');

			// элементы слайдера
			this.items = this.listServices.querySelectorAll('.stage > div');
		...
		init() {
			...
			this.options = ListServices.extend({}, ListServices.defaults, this.setup);
		...
		setSizeCarousel() {
			...
			setTimeout(() => { this.listServices.style.visibility = 'visible' }, 350);
		}
		...
		initScrollButtons() {
			...
			this.navCtrl = this.listServices.querySelector('.nav-button');

			// объект перелистывания галереи с помощью пагинации
			this.dotsCtrl = this.listServices.querySelector('.dots-slider');
		...
		registerEventsHandler(e) {
			...
			this.navCtrl.addEventListener('click', this.navScrollButtons.bind(this));

			// управление постраничной навигацией точками
			this.dotsCtrl.addEventListener('click', this.dotsScrollButtons.bind(this));
			...
			if (this.options.keyScrollButtons) {

				window.addEventListener('keydown', this.keyScrollButtons.bind(this));
			}

			// просмотр полноразмерной фотографии
			this.listServices.querySelector('.slider-services').addEventListener('click', this.showPhoto.bind(this));

			// mouse events
			// управление колёсиком мыши, управление работает, если указатель
			// мыши находится над DIV'ом с классом 'slider-services'
			this.listServices.querySelector('.slider-services').addEventListener('wheel', this.wheelScrollButtons.bind(this));
			...
			// курсор мыши выходит за пределы DIV'а с классом 'slider-services'
		...
		keyScrollButtons(e) {
			...
			// прекращаем работу функции
			if (e.which !== ListServices.RIGHT && e.which !== ListServices.LEFT) return;
			const d = (e.which === ListServices.RIGHT) ? 1 : -1;
		...
		tap(e) {
			...
			this.start = ListServices.xpos(e);
		...
		drag(e) {
			...
			this.shift = this.start - ListServices.xpos(e);
	...
	const listServicess = document.querySelectorAll('.list-services');
	// перебираем полученную коллекцию элементов
	for (let listServices of listServicess) {
		// вариант настроект для данной галереи
		const setup = listServices.dataset.setting;
		// создаём экземпляр галереи
		const slider = new ListServices(listServices, setting[setup]);
	}
})();